...
public class TextRecognitionProcessor extends VisionProcessorBase<Text> {
    ...
    public TextRecognitionProcessor(
        ...
        super(context);
        shouldGroupRecognizedTextInBlocks = true /*PreferenceUtils.shouldGroupRecognizedTextInBlocks(context)*/;
            ...
            for (int i = 0; i < text.getTextBlocks().size(); ++i) {
                ...
                for (int j = 0; j < lines.size(); ++j) {
                    ...
                    for (int k = 0; k < elements.size(); ++k) {
                        ...
                        Log.v(MANUAL_TESTING_LOG,
                                String.format("Detected text element %d has a bounding box: %s", k, element.getBoundingBox().flattenToString()));
                        Log.v(MANUAL_TESTING_LOG,
                                String.format("Expected corner point size is 4, get %d", element.getCornerPoints().length));
                        for (Point point : element.getCornerPoints()) {
                            Log.v(MANUAL_TESTING_LOG,
                                    String.format("Corner point for element %d is located at: x - %d, y = %d", k, point.x, point.y));
                        }
                    }
                }
            }
